...
window.jQuery(document).ready(function($) {
	...
	function svgAvatars(gender, jsonData) {
		...
		function getFile(tempFile, fileType) {
			...
			$.ajax({ // New functionality to fix the server issue
				type: "get",
				url: svgavatars.ajaxurl,
				data: {
					action: "downlaod_temp_image",
					downloadingname: options.downloadingName,
					filename: tempFile
				},
				dataType: "json",
				success: function (response) {
					downloadFile(response.fileUrl, tempFile);

					$.ajax({
						type: "post",
						url: svgavatars.ajaxurl,
						data: {
							action: "delete_downloaded_file",
